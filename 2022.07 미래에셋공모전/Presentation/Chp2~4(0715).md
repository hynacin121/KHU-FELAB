...
+ 리밸런싱 : 시가총액의 규모, 업종, PBR, PER, 배당 성향에따라 현금을 어떻게 배분할지 고민하는 작업
+ $PBR = \frac {시가총액} {순자산} $
+ $PER = \frac {시가총액} {당기순이익} $
...
+ 리밸런싱 : 가격변동에 따라 내재가치 증가시킴, 비중 조절이나 종목교체 발생
...
+ __정보비율(Information Ratio)__
    ...
    : 수익률을험 선호하고 위험을 회피한다는 가정하에 위험을 고려해 적극적인 투자성과 평가 <br>
...
- 상관계수가 낮은 자산을 결합하여 포트폴리오 구성할 수 있음을 제시
...
- 포트폴리오 리스크 : $\sum{w_i w_j \sigma_ij}$
- 포트폴리오 리스크 행렬연산 :<br>
$ w= \begin{pmatrix}w_1&  \dots &w_n\\ \end{pmatrix} \\
